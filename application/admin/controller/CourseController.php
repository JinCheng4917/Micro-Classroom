...
if (!is_null($Course))
{
    ...
    if (true !== $result)
    {
       ...
       $teacherIds = Request::instance()->post('teacher_id/a');
       $Course->save();
...
if (!is_null($teacherIds)) {
    if (!$Course->Teachers()->saveAll($teacherIds)) {
        return $this->error('课程-班级信息保存错误：' . $Course->Teachers()->getError());
    }
}
        // -------------------------- 新增班级课程信息(end) -------------------------- 
...
{ 
    ...
    if (!is_null($Course)){
        ...
        if (false === $Course->TeacherCourses()->where($map)->delete()) {
            return $this->error('删除教师课程关联信息发生错误' . $Course->TeacherCourses()->getError());
        }
        // 增加新增数据，执行添加操作。
        $klassIds = Request::instance()->post('klass_id/a');
        $teacherIds = Request::instance()->post('teacher_id/a');
